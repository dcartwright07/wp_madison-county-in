<template>
    ...
    <WhatsUpListingHeader :image="destination.image" />
		<!-- Content -->
					...
					<v-col class="col-md-7 col-lg-7 mb-3 pt-16">
						<div class="">
							<div class="detail-tile mb-4">
								...
								<p class="text-muted" v-html="destination.address + destination.city + ' ' + destination.state + ', '+ destination.zip"/>
								<v-btn disabled rounded  class="text-uppercase primary mr-2" v-for="(item) in destination.categories" :key="item.name">
									{{item.name}}
								</v-btn>
							</div>
							<div class="nav-wrapper">
								<v-tabs v-model="tab" class="nav nav-pills nav-fill flex-column flex-md-row" id="tabs-icons-text" role="tablist">
              ...
              <v-tabs-items v-model="tab">
                    <v-tab-item id="tabs-icons-text-1" >
										<div class="tab-pane fade active show" >
											<p class="description" v-html="destination.description"/>
                      {{destination}}
										</div>
                    </v-tab-item>
