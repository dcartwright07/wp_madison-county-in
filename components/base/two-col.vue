  ...
  <v-container fluid class="pa-0">
    ...
    >
      <v-col v-if="!category.posts.length < 1" cols="12" md="4" >
        <v-sheet rounded shaped class="icon-section transparent" :class="{'text-left': index % 2 === 0, 'text-right': index % 2 !== 0}">
          <div class="mb-5" :class="{
            'text-right': index % 2 !== 0,
            'text-left': index % 2 === 0,
            
          }">
            <h2 class="section-name mb-10" :class="{'text-right ml-2': index % 2 === 0, 'text-left mr-2': index % 2 !== 0}">
              {{ category.name }}
            </h2>
              
          </div>
          <div :class="{'text-right mr-4': index % 2 !== 0, 'text-left ml-4': index % 2 === 0}" v-html="category.content"></div>
          <nuxt-link
